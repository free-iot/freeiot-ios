...
- (void)_dispatchMessage:(WVJBMessage*)message {
    ...
    NSString* javascriptCommand = [NSString stringWithFormat:@"PandoJSBridge._handleMessageFromNative('%@');", messageJSON];
...
- (void)_sendMessage:(NSString*)message {
    ...
    NSString* javascriptCommand = [NSString stringWithFormat:@"PandoJSBridge._handleMessageFromNative('%@');", message];
...
- (void)_flushMessageQueue {
    ...
    //NSString *messageQueueString = @"ddddddddddd";
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([[url scheme] isEqualToString:kCustomProtocolScheme]) {
        if ([[url host] isEqualToString:kQueueHasMessage]) {
            ...
            //NSDictionary *login_data = [NSDictionary dictionaryWithObjectsAndKeys:@"", nil];
            
            dispatch_async(dispatch_get_main_queue(), ^{
                [strongDelegate WebViewJSBridge:self handleCostumProtocol:url];
            });
            
#if 0
            
            NSDictionary *inputData = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:10086],
                                       @"responseId",
            ...
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\\" withString:@"\\\\"];
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\"" withString:@"\\\""];
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\'" withString:@"\\\'"];
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\n" withString:@"\\n"];
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\r" withString:@"\\r"];
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\f" withString:@"\\f"];
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\u2028" withString:@"\\u2028"];
            jsonInputString = [jsonInputString stringByReplacingOccurrencesOfString:@"\u2029" withString:@"\\u2029"];
            
            
            NSString* javascriptCommand = [NSString stringWithFormat:@"PandoJSBridge._handleMessageFromNative('%@');", jsonInputString];
